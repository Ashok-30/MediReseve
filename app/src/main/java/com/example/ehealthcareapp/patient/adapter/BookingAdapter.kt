...
class BookingAdapter(private val bookingList : ArrayList<Booking> ) : RecyclerView.Adapter<BookingAdapter.MyViewHolder>() {
    private val dbRef: DatabaseReference = FirebaseDatabase.getInstance().getReference("Rating")

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {

        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.booking_data,
            parent,false)
        ...
        dbRef.orderByChild("bookingId").equalTo(currentItem.bookingId)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        // Booking Id exists in the Rating table
                        holder.rateButton.isEnabled = false
                        holder.rateButton.text = "Appointment Completed"
                    } else {
                        // Booking Id doesn't exist in the Rating table
                        holder.rateButton.isEnabled = true
                        holder.rateButton.text = "Rate"
                        holder.rateButton.setOnClickListener {
                            val context = holder.itemView.context
                            val intent = Intent(context, PatientRatingActivity::class.java)
